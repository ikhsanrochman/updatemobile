...
<div class="container-fluid">
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-body">
            <form action="{{ route('super_admin.documents.update', $document->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mb-3">
                ...
                <div class="mb-3">
                    <label for="file" class="form-label">File Dokumen (Biarkan kosong jika tidak ingin mengubah)</label>
                    <input type="file" class="form-control @error('file') is-invalid @enderror" id="file" name="file">
                    @error('file')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    @if ($document->file_path)
                        <div class="mt-2">File saat ini: <a href="{{ Storage::url($document->file_path) }}" target="_blank" class="text-decoration-none">Lihat File</a></div>
                    @endif
                </div>
                <div class="mb-3">
                ...
                <button type="submit" class="btn btn-dark-blue">Update Dokumen</button>
