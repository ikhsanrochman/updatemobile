                        ...
                        @forelse($dosisTlds->sortBy('tanggal_pemantauan') as $key => $dosisTld)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{ route('super_admin.pemantauan.tld.edit', ['projectId' => $project->id, 'userId' => $user->id, 'dosisId' => $dosisTld->id]) }}" class="btn btn-info btn-sm">
                                        <i class="fas fa-edit me-1"></i>Edit
                                    </a>
                                    <form action="{{ route('super_admin.pemantauan.tld.destroy', ['projectId' => $project->id, 'userId' => $user->id, 'dosisId' => $dosisTld->id]) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
